...
if [ $# -lt 2 ]
  then 
    echo " Usage  : autopcm.sh fname queue" 
    echo " Usage  : autopcm.sh fname queue charge"
    echo "     fname: output from spartan or genconf.sh"
    ...
    echo "       Any problem, pls conact anan@xmu.edu.cn"
    exit 1
fi
...
cat > head << EOF
%mem=10GB
#P PM7 OPT=(CalcFC,MaxCyc=280) Freq SCRF=(SMD,Solvent=Chloroform)
...
do
  sleep 60
  IFin=`qstat -u $USER | grep " "$pref"ge" | wc -l`  
done
...
cat > head << EOF
%mem=10GB
#P wB97XD/3-21G OPT=(ReadFC,MaxCyc=280) Freq SCRF=(SMD,Solvent=Chloroform)
...
cat > head << EOF
%mem=10GB
#P wB97XD/6-31G* OPT=(ReadFC,MaxCyc=280) Freq SCRF=(SMD,Solvent=Chloroform)
